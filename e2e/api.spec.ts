...
test.describe("API E2E", () => {
  ...
  test("GET /api/parcels/residential returns residential parcels", async ({
    request,
  }) => {
    ...
    // Each residential parcel must have category starting with R
    // or description starting with Residential
    for (const parcel of body.parcels) {
      ...
      const isResidential =
        ...
        (parcel.description &&
          parcel.description.startsWith("Residential"));
  ...
  test("GET /api/parcels/residential with invalid minArea returns 400", async ({
    request,
  }) => {
  ...
  test("GET /api/parcels/residential with negative minArea returns 400", async ({
    request,
  }) => {
    const response = await request.get(
      "/api/parcels/residential?minArea=-1",
    );
    expect(response.status()).toBe(400);
  ...
  test("GET /api/stats returns stats grouped by neighborhood", async ({
    request,
  }) => {
  ...
  test("GET /api/stats with invalid groupBy returns 400", async ({
    request,
  }) => {
